  ...
  <div class="overflow-x-auto sm:ml-6 shadow-md">
    <div class="min-w-full inline-block align-middle">
      <div class="rounded-lg overflow-visible bg-white p-6 dark:border-neutral-700">
        <form action="{{ route('surat-keluar.store') }}" class="mb-4" method="POST">
          @csrf
          ...
          <div class="grid gap-3">
            ...
            <div>
              ...
              <input type="text" name="hal" id="hal"
                ...
                placeholder="Masukkan hal" value="{{ old('hal') }}" autocomplete="off" />
              @error('hal')
                <x-validation>{{ ucfirst($message) }}</x-validation>
              @enderror
            </div>
            ...
            <div>
              ...
              <textarea name="isi_surat" id="isi_surat" placeholder="Masukkan isi surat disini...">{{ old('isi_surat') }}</textarea>
              @error('isi_surat')
                <x-validation>{{ ucfirst($message) }}</x-validation>
              @enderror
            </div>
