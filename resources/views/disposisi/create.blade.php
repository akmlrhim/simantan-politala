          ...
          <form action="{{ route('disposisi.store') }}" method="POST" class="space-y-5">
              ...
              <div class="flex flex-wrap gap-4">
                <label class="flex items-center space-x-2 cursor-pointer">
                  <input type="radio" name="tingkat_surat" value="biasa"
                    {{ old('tingkat_surat') == 'biasa' ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                  <span class="text-sm">Biasa</span>
                </label>

                <label class="flex items-center space-x-2 cursor-pointer">
                  <input type="radio" name="tingkat_surat" value="segera"
                    {{ old('tingkat_surat') == 'segera' ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                  <span class="text-sm">Segera</span>
                </label>

                <label class="flex items-center space-x-2 cursor-pointer">
                  <input type="radio" name="tingkat_surat" value="rahasia"
                    {{ old('tingkat_surat') == 'rahasia' ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                ...
                @foreach ($jabatan as $id => $item)
                  <label class="flex items-center space-x-1 cursor-pointer">
                    <input type="checkbox" name="kepada_jabatan_id[]" value="{{ $id }}"
                      {{ is_array(old('kepada_jabatan_id')) && in_array($id, old('kepada_jabatan_id')) ? 'checked' : '' }}
            ...
            <div>
              ...
              <div
                ...
                @foreach ($instruksiList as $instruksi)
                  <label class="flex items-center space-x-1 cursor-pointer">
                    <input type="checkbox" name="instruksi_disposisi[]" value="{{ $instruksi }}"
                      {{ in_array($instruksi, old('instruksi_disposisi', [])) ? 'checked' : '' }}
                      class="text-blue-600 focus:ring-blue-500 rounded">
                ...
                @php
                  $oldInstruksi = old('instruksi_disposisi', []);
                @endphp

                @for ($i = 0; $i < 3; $i++)
                  @php
                    $manualVal = $oldInstruksi[count($instruksiList) + $i] ?? '';
                  @endphp
                  <label class="flex items-center space-x-1 cursor-pointer">
                    ...
                    <input type="text" name="instruksi_disposisi[]" placeholder="Instruksi lain..."
                      value="{{ $manualVal }}" class="text-xs border-gray-300 rounded p-1 disabled:bg-gray-100"
                      {{ $manualVal ? '' : 'disabled' }}>
                  </label>
                @endfor
            ...
            <div>
              ...
              <textarea name="catatan" rows="5" placeholder="Masukkan catatan (opsional..)"
                class="w-full text-sm font-medium border-gray-300 rounded-l">{{ old('catatan') }}</textarea>
            </div>
